...
//
// Rendering goes through cc.MeshRenderer components — no manual
// updateTransform() pumping; the component's lateUpdate pushes transforms.
...
private:
    Node *makeBody(const char *nodeName, Mesh *mesh, Material *material, Node *parent = nullptr) {
    ...
    void makeGround() {
        ...
        p.roughness = 0.95F;
        Node *ground = makeBody("ground", game::PrimitiveFactory::createBox(9.F, 0.2F, 5.F),
    ...
    void makeOrbiter() {
        ...
        p.roughness = 0.35F;
        p.metallic = 0.6F;
        makeBody("torus", game::PrimitiveFactory::createTorus(0.45F, 0.14F),
    ...
    void makeBouncer() {
        ...
        p.roughness = 0.5F;
        Node *ball = makeBody("ball", game::PrimitiveFactory::createSphere(0.5F),
                              game::MaterialFactory::createStandard(p));
        ...
        auto *animComp = ball->addComponent<AnimationComponent>();
        animComp->addClip(clip);
        auto *state = animComp->play();
    ...
    void makePopper() {
        ...
        p.roughness = 0.25F;
        p.metallic = 0.8F;
        Node *cone = makeBody("cone", game::PrimitiveFactory::createCone(0.5F, 1.1F),
                              game::MaterialFactory::createStandard(p));
        cone->setPosition(-2.6F, -0.5F, 0.F);
        ...
        _popperAnim = cone->addComponent<AnimationComponent>();
